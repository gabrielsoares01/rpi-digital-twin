...
## 4. Issue proposta — gabrielsoares01/rpi-digital-twin #18

**Título:** Exportar histórico de telemetria em CSV
**Link:** https://github.com/gabrielsoares01/rpi-digital-twin/issues/18
**Estado:** Aberta
...
**Resumo:** 6 issues abertas: focadas em maturidade de engenharia (observabilidade, testes automatizados) e qualidade de documentação (rastreabilidade requisitos↔arquitetura, critérios de aceite), mais 1 bug de performance no frontend (stream de telemetria travando a UI). A #16 e a #18 (CI/CD e export como .csv) estão listadas separadamente por serem feedbacks de outros grupos, não uma proposta própria. As demais entradas do histórico (#1–#9) são pull requests já mescladas (dashboard mockado, backend de sensores, digital twin 3D, sidebar, hotspot Wi-Fi, correções de stutter/buffer, release da semana 1) e não representam propostas em aberto.
